...
def check_entity_files(entity, files_field=True):
    ...
    assert set(fi["title"][default_lang_code] for fi in file_list) == set(
        FILES.values()
    )
...
def test_POST_file_multipart_root_endpoint(john_smith_api_client, default_hearing):
    ...
    with open(get_image_path(IMAGES["ORIGINAL"]), "rb") as fp:
        post_data["file"] = fp
        data = get_data_from_response(
            john_smith_api_client.post("/v1/file/", data=post_data, format="multipart"),
            status_code=201,
        )
...
def test_POST_file_base64_root_endpoint(john_smith_api_client, default_hearing):
    ...
    post_data["section"] = first_section["id"]
    data = get_data_from_response(
        john_smith_api_client.post("/v1/file/", data=post_data), status_code=201
    )
    # Save order of the newly created file
    ordering = data["ordering"]
    # Make sure new file was created
...
def test_unpublished_section_files_excluded(client, expected, request, default_hearing):
    ...
    file_obj = default_hearing.sections.all()[2].files.get(
        translations__title=FILES["PDF"]
    )
    ...
    for set_num, file_set in zip(range(1, 3), (file_set_1, file_set_2)):
        assert (
            FILES["PDF"]
            in [file_obj["title"][default_lang_code] for file_obj in file_set]
        ) is expected, "Set %d failed" % set_num
...
@pytest.mark.skip
@pytest.mark.django_db
def test_unpublished_hearing_section_files_download_link_admin(
...
def test_PUT_file_multipart_section(john_smith_api_client, default_hearing):
    ...
    data = get_data_from_response(
        john_smith_api_client.put(
            "/v1/file/%s/" % file_obj_id, data=put_data, format="multipart"
        ),
        status_code=200,
    )
...
def test_PUT_file_json_section(john_smith_api_client, default_hearing):
    ...
    data = get_data_from_response(
        john_smith_api_client.put("/v1/file/%s/" % file_obj_id, data=data),
        status_code=200,
    )
    assert data["section"] == first_section["id"]
    assert data["hearing"] == default_hearing.pk
...
def test_PUT_file_no_access(john_doe_api_client, default_hearing):
    ...
    put_data = sectionfile_multipart_test_data()
    get_data_from_response(
        john_doe_api_client.put(url, data=put_data, format="multipart"), status_code=403
    )


@pytest.mark.parametrize(
    "client, expected",
    [("api_client", 401), ("jane_doe_api_client", 403), ("john_smith_api_client", 204)],
)
@pytest.mark.django_db
...
def test_POST_first_file(john_smith_api_client, default_hearing):
    ...
    with open(get_file_path(FILES["PDF"]), "rb") as fp:
        post_data["file"] = fp
        data = get_data_from_response(
            john_smith_api_client.post(url, data=post_data, format="multipart"),
            status_code=201,
        )
...
def test_ckeditor_file_upload_orphan(admin_api_client_logged_in):
    ...
    with open(get_file_path(FILES["PDF"]), "rb") as fp:
        post_data["upload"] = fp
        response = admin_api_client_logged_in.post(
            url, data=post_data, format="multipart"
        )
    assert response.status_code == 200, (
        "expected status_code 200, received %s" % response.status_code
    )
    assert SectionFile.objects.count() == 1
    sectionfile_id = SectionFile.objects.first().pk
    expected = (
        r"window.parent.CKEDITOR.tools.callFunction\(1, 'https?://.+/v1/download/sectionfile/%s/'\);"
        % sectionfile_id
    )
...
def test_ckeditor_get_uploaded_orphan(admin_api_client_logged_in):
    ...
    ), "expected status_code 200 when accessing uploaded file"
...
def test_bind_orphan_file_to_section(default_hearing, section_file_orphan):
    ...
    download_url = reverse(
        "serve_file", kwargs={"filetype": "sectionfile", "pk": section_file_orphan.pk}
    )
...
@pytest.mark.django_db
def test_file_id_is_audit_logged_on_retrieve(
...
@pytest.mark.django_db
def test_file_id_is_audit_logged_on_create(
...
@pytest.mark.django_db
def test_file_id_is_audit_logged_on_update(
    john_smith_api_client, default_hearing, audit_log_configure
):
    post_data = sectionfile_multipart_test_data()
    url = reverse("file-list")
    ...
    john_smith_api_client.put(
        reverse("file-detail", kwargs={"pk": file_obj_id}),
        data=put_data,
        format="multipart",
    )
...
@pytest.mark.django_db
def test_file_id_is_audit_logged_on_delete(
    john_smith_api_client, default_hearing, audit_log_configure
):
    file_obj = default_hearing.sections.first().files.first()
