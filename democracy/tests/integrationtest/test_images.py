...
def check_entity_images(entity, images_field=True):
    ...
    assert set(im["title"][default_lang_code] for im in image_list) == set(
        IMAGES.values()
    )
...
def test_38_get_hearing_check_section_with_images(api_client, default_hearing):
    ...
    """
    data = get_data_from_response(
        api_client.get(get_hearing_detail_url(default_hearing.id))
    )
...
@pytest.mark.xfail(
    reason="sporadic failures, race condition suspected, needs debugging"
)
...
@pytest.mark.django_db
def test_unpublished_section_images_excluded(
    client, expected, request, default_hearing
):
    ...
    image = default_hearing.sections.all()[2].images.get(
        translations__title=IMAGES["ORIGINAL"]
    )
    image.published = False
    image.save(update_fields=("published",))
    ...
    if expected:
        assert (
            main_image["title"][default_lang_code]
            == default_hearing.get_main_section().images.first().title
        )
    else:
    ...
    response = api_client.get(
        f"{reverse('image-list')}?section={default_hearing.sections.all()[2].id}"
    )
    ...
    for image_set in (image_set_1, image_set_2, image_set_3):
        assert (
            IMAGES["ORIGINAL"]
            in [image["title"][default_lang_code] for image in image_set]
        ) is expected
...
def test_get_images_root_endpoint(api_client, default_hearing):
    ...
    data = get_data_from_response(
        api_client.get(
            f"{reverse('image-list')}?section={default_hearing.sections.first().id}"
        )
    )
...
def test_get_thumbnail_image(api_client, default_hearing):
    detail_url = reverse(
        "image-detail",
        kwargs={"pk": default_hearing.sections.first().images.first().id},
    )
...
def test_root_endpoint_filters(api_client, default_hearing, random_hearing):
    ...
    create_default_images(
        random_hearing.sections.exclude(type__identifier="main").first()
    )
...
def test_POST_image_root_endpoint(john_smith_api_client, default_hearing):
    ...
    data = get_data_from_response(
        john_smith_api_client.post(
            reverse("image-list"), data=post_data, format="json"
        ),
        status_code=201,
    )
...
def test_POST_image_root_endpoint_wrong_user(john_doe_api_client, default_hearing):
    # Get some section
    data = get_data_from_response(
        john_doe_api_client.get(get_hearing_detail_url(default_hearing.id, "sections"))
    )
    first_section = data[0]
    # POST new image to the section
    post_data = sectionimage_test_json()
    post_data["section"] = first_section["id"]
    data = get_data_from_response(
        john_doe_api_client.post(reverse("image-list"), data=post_data, format="json"),
        status_code=403,
    )
...
def test_PATCH_image_root_endpoint(john_smith_api_client, default_hearing):
    ...
    changed_section_image = get_data_from_response(
        john_smith_api_client.get(detail_url)
    )
...
def test_PATCH_image_root_endpoint_wrong_user(john_doe_api_client, default_hearing):
    ...
    data = get_data_from_response(
        john_doe_api_client.patch(
            reverse("image-detail", kwargs={"pk": section_image["id"]}),
            data=post_data,
            format="json",
        ),
        status_code=403,
...
def test_DELETE_image_root_endpoint(john_smith_api_client, default_hearing):
    ...
    response = john_smith_api_client.delete(
        reverse("image-detail", kwargs={"pk": section_image["id"]}), format="json"
    )
...
@pytest.mark.django_db
def test_image_id_is_audit_logged_on_retrieve(
...
@pytest.mark.django_db
def test_image_ids_are_audit_logged_on_list(
    api_client, default_hearing, audit_log_configure
):
    url = reverse("image-list")
    ...
    assert_audit_log_entry(
        url, section_images.values_list("pk", flat=True), operation=Operation.READ
    )


@pytest.mark.django_db
def test_image_id_is_audit_logged_on_create(
...
@pytest.mark.django_db
def test_file_id_is_audit_logged_on_update_PATCH(
    john_smith_api_client, default_hearing, audit_log_configure
):
...
@pytest.mark.django_db
def test_file_id_is_audit_logged_on_update_PUT(
    john_smith_api_client, default_hearing, audit_log_configure
):
    section_image = default_hearing.sections.first().images.first()
    url = reverse("image-detail", kwargs={"pk": section_image.pk})
...
@pytest.mark.django_db
def test_image_id_is_audit_logged_on_delete(
    john_smith_api_client, default_hearing, audit_log_configure
):
    section_image = default_hearing.sections.first().images.first()
    url = reverse("image-detail", kwargs={"pk": section_image.pk})
