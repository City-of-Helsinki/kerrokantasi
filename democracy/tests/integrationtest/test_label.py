...
def test_cannot_post_label_without_authentication(api_client, valid_label_json):
    response = api_client.post(reverse("label-list"), data=valid_label_json, format="json")
...
def test_cannot_post_label_without_authorization(john_doe_api_client, valid_label_json):
    response = john_doe_api_client.post(reverse("label-list"), data=valid_label_json, format="json")
...
def test_admin_user_can_post_label(john_smith_api_client, valid_label_json):
    response = john_smith_api_client.post(reverse("label-list"), data=valid_label_json, format="json")
    data = get_data_from_response(response, status_code=201)
    assert data["label"] == valid_label_json["label"]


@pytest.mark.django_db
def test_label_id_is_audit_logged_on_retrieve(api_client, random_label, audit_log_configure):
...
@pytest.mark.django_db
def test_label_ids_are_audit_logged_on_list(api_client, random_label, audit_log_configure):
...
@pytest.mark.django_db
def test_label_id_is_audit_logged_on_create(api_client, john_smith_api_client, valid_label_json, audit_log_configure):
    url = reverse("label-list")
