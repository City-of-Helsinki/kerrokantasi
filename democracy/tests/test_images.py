...
@pytest.mark.django_db
def test_POST_image_root_endpoint(admin_api_client, default_hearing):
    # Check origninal image count
    data = get_data_from_response(admin_api_client.get('/v1/image/'))
    assert len(data['results']) == 9
    # Get some section
    data = get_data_from_response(admin_api_client.get(get_hearing_detail_url(default_hearing.id, 'sections')))
    first_section = data[0]
    # POST new image to the section
    post_data = sectionimage_test_json()
    post_data['section'] = first_section['id']
    data = get_data_from_response(admin_api_client.post('/v1/image/', data=post_data, format='json'), status_code=201)
...
@pytest.mark.django_db
def test_PATCH_image_root_endpoint(admin_api_client, default_hearing):
    data = get_data_from_response(admin_api_client.get('/v1/image/'))
    section_image = data['results'][0]
    post_data = {'title': {'en': 'changed_title'}, 'caption': {'en': 'changed_caption'}}
    data = get_data_from_response(admin_api_client.patch('/v1/image/%d/' % section_image['id'], data=post_data, format='json'), status_code=200)
    changed_section_image = get_data_from_response(admin_api_client.get('/v1/image/%d/' % section_image['id']))
    assert changed_section_image['title']['en'] == 'changed_title'


@pytest.mark.django_db
def test_DELETE_image_root_endpoint(admin_api_client, default_hearing):
    # Check origninal image count
    data = get_data_from_response(admin_api_client.get('/v1/image/'))
    assert len(data['results']) == 9
    section_image = data['results'][0]
    response = admin_api_client.delete('/v1/image/%d/' % section_image['id'], format='json')
    assert response.status_code == 204
    data = get_data_from_response(admin_api_client.get('/v1/image/%d/' % section_image['id'], format='json'), status_code=404)
    data = get_data_from_response(admin_api_client.get('/v1/image/'))
