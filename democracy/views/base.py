    ...
    def get_queryset(self):
        if not (self.model and issubclass(self.model, BaseModel)):  # pragma: no cover
            raise ImproperlyConfigured(
                "AdminsSeeUnpublishedMixin requires `model` to be a BaseModel subclass (it's %r)"
                % self.model
            )
        ...
        if user is None:  # pragma: no cover
            raise ImproperlyConfigured(
                "%r has no request or serialization context; AdminsSeeUnpublishedMixin requires one"
                % self
            )
