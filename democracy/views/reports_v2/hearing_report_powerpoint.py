...
from democracy.views.reports_v2.utils import (get_default_translation,
    get_formatted_hearing_timerange, get_powerpoint_title_font_size, get_selected_language)
...
class HearingReportPowerPoint():
    ...
    def _add_main_title_slide(self):
        ...
        info.text = (f'{_("Kerrokantasi hearing")} {hearing_timerange}'
    ...
    def _add_comment_slides(self, comments: dict):
        ...
        for comment in comments:
            ...
            comment_rows = int(
                math.ceil(len(current_comment['content']) / MAX_CHARACTERS_PER_SINGLE_COMMENT_ROW))
    ...
    def _add_poll_slide(self, poll: dict):
        ...
        for option in poll_options:
            options_data['categories'].append(get_default_translation(
                option['text'], self.used_language))
    ...
    def get_response(self):
        ...
        response['Content-Disposition'] = 'attachment; filename="{filename}.pptx"'.format(
            filename=re.sub(r"\W+|_", " ", get_default_translation(
                self.json['title'], self.used_language)))
