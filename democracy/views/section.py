...
class SectionCreateUpdateSerializer(
    ...
    def _validate_question_update(self, poll_data, poll):
        ...
        poll_changed = (
            not compare_serialized(old_poll_data["text"], poll_data["text"])
            or len(old_poll_data["options"]) != len(poll_data["options"])
            or any(
                not compare_serialized(old_option["text"], option["text"])
