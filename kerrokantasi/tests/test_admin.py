...
@pytest.mark.django_db
def test_user_admin_admin_in_organizations_field_single_org(
    user_admin, user, organization
):
...
@pytest.mark.django_db
def test_user_admin_admin_in_organizations_field_multiple_orgs(
    user_admin, user, organization_factory
):
    ...
    assert (
        user_admin.admin_in_organizations(user)
        == f"{organization1.name}, {organization2.name}"
    )
